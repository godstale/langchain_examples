...
# 1-2. 테스트 대상이 되는 뉴스 URLs, 데이터 임시 저장용 캐시 생성
news_urls = [
...
for news in news_array:
    ...
    # 4. 새로 추가된 example 조회 (create_examples() 이 추가한 내용을 리턴하지 않음)
    # new_examples = list(
    #     client.list_examples(dataset_id=ds.id, example_ids=[example_uuid])
    # )
    ...
    # 7. feedback 받기 위해 run_on_dataset 결과를 annotation queue에 추가
    ...
    # run_id로 조회하면 하나의 annotation queue 항목마다 평가항목별로 feedback이 생성됨
    # run_id로 그룹화해서 리턴해야함
    ...
    # 10. 현재 프롬프트, 기사 원문, 요약 결과, 피드백 데이터를
    #   모두 넣어서 프롬프트 구성 후 실행
